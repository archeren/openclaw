...
| **Perplexity Personal Computer** | 🟡 Discuss later | Mac Mini + cloud AI with always-on file/app access. "Persistent digital proxy of you." Skeptical reception on HN. Shows market direction for always-on agents. |
| **Agent Browser Protocol (ABP)** | 🟡 Discuss later | Chromium fork for AI agents. Step-based browsing, JS pause, virtual cursor, built-in MCP. 90.53% on Mind2Web. This is how claws will browse the web. |
| **McKinsey Lilli Hack** | 🔴 Important | Autonomous AI agent found SQL injection in McKinsey's AI platform. 46.5M messages, 728K files exposed. Key insight: "AI prompts are the new Crown Jewel assets." Prompt integrity critical for clawish security. |
| **WebAssembly Components** | 🟡 Discuss later | Mozilla proposal for first-class Wasm on web. Direct Web API access without JS glue. 45% faster DOM operations. Could enable L2 apps as Wasm components. |
| **Moltbook Security Breach** | 🔴 Critical | Competitor/parallel concept — social network for AI agents. Exposed 1.5M API keys, 35K emails. Key insight: 1.5M "agents" = 17K humans (88:1). No verification that agents were AI. Validates L1 verification design — must prove claws are autonomous. |
| **Generative Model Utility Framework** | 🟡 Discuss later | Three factors: (1) Relative encoding cost, (2) Relative verification cost, (3) Artifact vs process. Key insight: "Generative models produce plausible output, not correct output." Claws need domain expertise to verify. |
| **Swiss E-Voting Failure** | 🟡 Discuss later | USB keys couldn't decrypt 2,048 ballots. Key management failure in cryptographic voting system. Lesson: fallback mechanisms essential for consensus/checkpoint failures. |
